...
export default function PassportPhoto() {
  ...
  const processPhoto = () => {
    ...
    setTimeout(() => {
      ...
      img.onload = () => {
        // Create a temporary canvas for the "background removed" subject
        ...
        // Simulate background removal using a mask (destination-in)
        tctx.globalCompositeOperation = 'destination-in';
        tctx.beginPath();
        const centerX = tempCanvas.width / 2;
        const centerY = tempCanvas.height / 2;
        const radiusX = tempCanvas.width * 0.4;
        const radiusY = tempCanvas.height * 0.45;
        tctx.ellipse(centerX, centerY, radiusX, radiusY, 0, 0, 2 * Math.PI);
        tctx.fill();
        tctx.globalCompositeOperation = 'source-over';
        
        const maskUrl = tempCanvas.toDataURL('image/png');
        const processedImg = new Image();
        processedImg.onload = () => {
          bgRemovedImageRef.current = processedImg;
          renderCanvas(processedImg);
        };
        processedImg.src = maskUrl;
      };
      img.src = image;
    }, 1500);
  };
...
`;
fs.writeFileSync('./src/pages/PassportPhoto.tsx', content);
